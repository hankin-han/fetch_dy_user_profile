...
在抖音用户主页，通过 Chrome 开发者工具控制台注入此脚本，可获取该用户所有公开作品的结构化数据，并在页面侧边栏中以多种视图展示（表格/网格/仪表盘/对比/时间线），支持搜索、筛选、排序、预览，并支持导出为 JSON / CSV / TXT / 视频 ZIP 四种格式。
...
```
/
└── fetch_dy_user_profile.js   # 主脚本文件（约 2700 行）
```
